...
function formatMarkdown(text) {
  ...
  let processed = text.replace(/```(\w*)\n([\s\S]*?)```/g, (_, lang, code) => {
    ...
    const copyBtn = `<button onclick="navigator.clipboard.writeText(this.parentElement.querySelector('code').textContent).then(()=>{this.textContent='Copied!';setTimeout(()=>this.textContent='Copy',1500)})" style="position:absolute;top:4px;right:4px;padding:2px 8px;font-size:11px;background:var(--background-modifier-hover,rgba(255,255,255,0.1));border:1px solid var(--background-modifier-border,rgba(255,255,255,0.1));border-radius:3px;color:var(--text-muted,#999);cursor:pointer;opacity:0;transition:opacity 0.15s">Copy</button>`;
